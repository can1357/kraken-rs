...
- Opening a repository paints from LMDB before Git runs. `SnapshotCache` (`src/git/cache.rs`) keeps the last delivered snapshot per repository; the worker ships it as `GitPayload::Cached` ahead of the real walk. That data is provisional: `AppState::provisional` blocks mutations and detail/diff jobs, and recents/selection/detail prefetch wait until a real `GitPayload::Snapshot` promotes it. This is the only `#[allow(unsafe_code)]` in the tree (LMDB's mmap open).
